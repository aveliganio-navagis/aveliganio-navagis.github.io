$(function () {
    ...
    function loadStores() {
        $.each(window.dataStore, function (index, item) {
    ...
    $('.nearby-stores').on('click', function () {
        // Show Radius Modal
        $('#radius-panel').openModal({
            dismissible: false,
            opacity: 0
        });
        // a hack to allow map from panning
    ...
    $('.store-distance').on('click', function () {
        $.when(getCurrentLocation()).then(
            function (status) {
                $('#distance-panel').openModal({
                    dismissible: false,
                    opacity: 0
                });
                // a hack to allow map from panning
